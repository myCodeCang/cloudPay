...
//		dc.createAlias("category.site", "category.site");
		if (article.getCategory()!=null && StringUtils2.isNotBlank(article.getCategory().getId()) && !Category.isRoot(article.getCategory().getId())){
...
//		if (StringUtils2.isBlank(page.getOrderBy())){
...
//		return dao.find(page, dc);
	...
	public void save(Article article) throws ValidationException {
		...
		if (article.getCategory()!=null&& StringUtils2.isNotBlank(article.getCategory().getId())){
			Category category = categoryDao.get(article.getCategory().getId());
        ...
        if (StringUtils2.isNotBlank(article.getViewConfig())){
            article.setViewConfig(StringEscapeUtils.unescapeHtml4(article.getViewConfig()));
        }
        
        ArticleData articleData = new ArticleData();;
		if (StringUtils2.isBlank(article.getId())){
			article.preInsert();
	...
	@Transactional(readOnly = false)
	public void delete(Article article, Boolean isRe)throws ValidationException {
//		dao.updateDelFlag(id, isRe!=null&&isRe?Article.DEL_FLAG_NORMAL:Article.DEL_FLAG_DELETE);
	...
	public List<Object[]> findByIds(String ids) {
		...
		String[] idss = StringUtils2.split(ids,",");
		Article e = null;
		for(int i=0;(idss.length-i)>0;i++){
			e = dao.get(idss[i]);
			list.add(new Object[]{e.getCategory().getId(),e.getId(), StringUtils2.abbr(e.getTitle(),50)});
		}
		return list;
	}
...
//		}
//		
//		if (StringUtils2.isNotBlank(beginDate) && StringUtils2.isNotBlank(endDate)) {
...
//		System.out.println(queryFilter);
	...
	public Article getArticleDataByKeyword(String keywords){
		Article article = dao.getArticleDataByKeyword(keywords);
		article.setArticleData (articleDataDao.get (article.getId ()));
		return article;
	}
	public Article getArticleDataById(String id){
		Article article = dao.get(id);
		article.setArticleData (articleDataDao.get (id));
		return article;
	}
}
