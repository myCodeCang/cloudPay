...
<%@ attribute name="isUsercenter" type="java.lang.String" required="false" description="选择框标题"%>
<%@ attribute name="userLevelId" type="java.lang.String" required="false" description="选择框标题"%>
...
<div class="input-append">
	<input id="${id}Id" name="userUserinfo.parentListLike" class="${cssClass}" type="hidden" />
...
<script type="text/javascript">
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/user/userUserinfo/selectUserTagList?pageSize=8${isUsercenter?"&isUsercenter=1":""}&userLevelId=${userLevelId}", "选择${title}", 750, 500, {
			ajaxData:{selectIds: ""},buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				...
				if (v=="ok"){
                    ...
                    var id =  $(table).find(".selected").attr('dataId');
					$("#${id}").val(username);
					$("#${id}Id").val(id);
				}
				else if (v=="clear"){
