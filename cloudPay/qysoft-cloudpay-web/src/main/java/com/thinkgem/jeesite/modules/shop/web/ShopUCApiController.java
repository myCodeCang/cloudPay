...
public class ShopUCApiController extends BaseController {
    ...
    public String updateUserMoney(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (!StringUtils2.isBlank(shopId)) {
            if(paramMoneyType.equals(EnumUtil.MoneyChangeType.wxshop.toString())){
                changeType = EnumUtil.MoneyChangeType.charget;
            }
            if(paramMoneyType.equals(EnumUtil.MoneyChangeType.WXSHOP_CONSUME.toString())){
                changeType = EnumUtil.MoneyChangeType.CONSUMPTION;
                message = "商城" + message;
            }
            //changeType = EnumUtil.MoneyChangeType.charget;
        ...
        try {
            userUserinfoService.updateUserMoney(userInfo.getUserName(), new BigDecimal(money), message, changeType, params);
            ...
            if (syncScore.isPresent() && "on".equals(syncScore.get())) {
                ...
                if (syncScoreField.isPresent()) {
                    ...
                    if (EnumUtil.MoneyType.score.toString().equals(syncFile)) {
                        userUserinfoService.updateUserScore(userInfo.getUserName(), new BigDecimal(money), "商城积分"+message, changeType);
                    } else {
                        Optional<EnumUtil.MoneyType> instanceByCode = EnumUtil.MoneyType.getInstanceByCode(syncFile);
                        if (instanceByCode.isPresent()) {
                            userUserinfoService.updateUserOtherMoney(userInfo.getUserName(), new BigDecimal(money), instanceByCode.get(), message, changeType);
                        }
                    }
                }
            }
    ...
    public String buyOrder(HttpServletRequest request, HttpServletResponse response, Model model) {
       ...
       userInfoExtYzfService.updateUserSilu(userUserinfoService.getByMobile(mobile).getUserName(),new BigDecimal(orderMoney),"商城消费", EnumUtil.MoneyChangeType.WXSHOP_CONSUME,true);
        return success("成功!!", response, model);
    }
